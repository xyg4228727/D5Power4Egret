...
//            this._tiledResource = {};
        ...
        private setupRoad(res:ArrayBuffer):void {
            ...
            if(res==null || res==undefined)
            {
                ...
                var value:number;
                if (sign == 'D5Map') {
                    var py:number = data.readShort();
                    var px:number = data.readShort();
                    ...
                    var h:number = Math.floor(this._mapHeight / this._roadH);
                    var w:number = Math.floor(this._mapWidth / this._roadW);
                    var k:number = w == px && h == py ? 1 : (px - 1) / (w - 1);

                    //console.log(px, py, w, h, 'd5mAP');
                    for (y = 0; y < h; y++) {
                        for (x = 0; x < w; x++) {
                            value = resmap[Math.floor(y * k)][Math.floor(x * k)];
